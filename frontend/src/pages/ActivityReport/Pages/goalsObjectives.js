...
const GoalsObjectives = () => {
  ...
  const granteeIds = recipientGrantee ? recipients.map((r) => r.grant.grantee.id) : [];
  const singleGrantee = new Set(granteeIds).size === 1;

  useDeepCompareEffect(() => {
    const fetch = async () => {
      if (recipientGrantee && hasGrants && singleGrantee) {
