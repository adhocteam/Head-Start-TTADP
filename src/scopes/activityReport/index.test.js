...
describe('filtersToScopes', () => {
  ...
  beforeAll(async () => {
    ...
    globallyExcluded = await ActivityReport.create({ ...draftReport, updatedAt: '2000-01-01' }, { silent: true });
  });
  ...
  //     reportIncludedLegacy = await ActivityReport.create({ ...draftReport, legacyId: 'R01-AR-012345' });
  ...
  describe('calculatedStatus', () => {
    let includedReportSingleApprover;
    ...
    beforeAll(async () => {
      includedReportSingleApprover = await ActivityReport.create(approvedReport);
      includedReportMultApprover = await ActivityReport.create(submittedReport);
      approverIn = await ActivityReportApprover.create({
        ...approverApproved,
        activityReportId: includedReportMultApprover.id,
      });

      console.log('includedReportSingleApprover > ', includedReportSingleApprover.toJSON());
      console.log('includedReportMultApprover > ', includedReportMultApprover.toJSON());
      console.log('approverIn >', approverIn.toJSON());

      // This hits the getter, returns calculatedStatus of approved
      console.log('includedReportSingleApprover.calculatedStatus > ', includedReportSingleApprover.calculatedStatus);

      // This hits the getter, status
      console.log('includedReportMultApprover.calculatedStatus > ', includedReportMultApprover.calculatedStatus);

      excludedReportMultApprover = await ActivityReport.create(submittedReport);
      approverOut = await ActivityReportApprover.create({
        ...approverRejected,
        activityReportId: excludedReportMultApprover.id,
      });
      possibleIds = [
        includedReportSingleApprover.id,
